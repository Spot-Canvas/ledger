...
**Non-Goals:**
- Updating the backtester to simulate the new layered exit logic (tracked as follow-up).
...
1. Add migration: `ALTER TABLE engine_position_state ADD COLUMN hard_stop NUMERIC, ADD COLUMN granularity TEXT`.
2. Deploy new engine binary. Existing positions pick up the NULL-safe defaults.
3. No rollback concern — the new columns are nullable and additive; rolling back the binary leaves orphaned columns that do not affect existing queries.
