...
#### Scenario: Config show includes new platform keys
- **WHEN** `trader config show` is run on a fresh install
- **THEN** the table SHALL include `api_url`, `web_url`, `ingestion_url`, `nats_url`, and `nats_creds_file` rows
- **AND** keys with defaults SHALL show `[default]` as their source
...
- **WHEN** `trader config set trader_url https://my-ledger.example.com` is run
- **THEN** the value SHALL be written to `~/.config/trader/config.yaml`
